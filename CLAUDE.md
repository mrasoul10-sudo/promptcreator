...
- **Getting the app:** «دریافت اپ اندروید» in the sidebar and the account menu opens `#/app`; Android browsers also get a dismissible top banner (hidden 30 days). None of these show inside the Android app (user agent contains `PromptSazApp`). The APK URL lives in `config.js`.
- **Rail tooltips animate opacity only.** A tooltip that moves while appearing can overflow the viewport for a frame, which makes a scrollbar appear, shifts the layout and loses hover: an endless flicker. There is a regression test for it.
