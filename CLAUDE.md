...
- **CSP** in `index.html` allows scripts from `'self'` only, styles from `'self'` and Google Fonts, and `connect-src` to `https://api.anthropic.com`. Do not add inline `<script>`, inline `style="..."` attributes in HTML strings, or `eval`. Set dynamic styles via the CSSOM (`el.style.x = ...`) instead.
...
assets/js/auth.js       Local accounts (PBKDF2), session, profile/settings, avatar processing
...
- Routes are `#/studio`, `#/history`, `#/archive`, `#/profile`, `#/settings`, `#/login`, `#/register`. Filters are mirrored into the hash query string (bookmarkable) via `syncQuery()` without re-routing.
