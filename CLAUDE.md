...
- **Brand:** one color, `--brand: #6C4CF5`. The mark is a round speech bubble whose tail forms a «P», with a robot head inside (antenna, two eyes, small bubble tail): `assets/img/logo.svg` and `logoMark()` in `ui.js` (colors from CSS classes `logo-bubble`/`logo-head`/`logo-eye`), used plain inside the app. `app-icon.svg`/`favicon.svg` put the white mark on a brand-color rounded square; `logo-maskable.svg` is the full-bleed version. Keep each shape its own `<path>` (overlapping subpaths in one path punch holes). After editing, run `node tools/render-icons.mjs` to regenerate every PNG (web and Android).
