...
assets/img/             logo.svg (blue split-ring logo), app-icon.svg + favicon.svg (logo on a white rounded square), logo-maskable.svg, icons/ (PNGs from tools/render-icons.mjs)
...
- **Updates (`updates.js`):** `CHANGELOG` (newest first; add an entry with the next `id` for every user-visible release) drives the one-time «تازه‌های این نسخه» popup for returning users and «تازه‌ها» in the account menu/help page. In the app: `maybeShowAppWhatsNew()` shows what the installed version brings once per version code (after an update, or «به پرامپت‌ساز خوش آمدید» on a fresh install), then `checkAppUpdate()` compares the UA version with `assets/android-version.json` (written and committed by `android.yml`): the dialog opens (unless «بعداً» in the last day), and a «به‌روزرسانی» button (`.tb-update`) stays in the top bar while a newer APK exists; it re-checks when the app returns to the foreground. Settings has «نسخه اپ» with a manual check.
