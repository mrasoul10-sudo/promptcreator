...
- **CSP** in `index.html` allows scripts from `'self'` plus Google's GSI client, styles from `'self'`, Google Fonts and GSI, frames from `accounts.google.com` only, and `connect-src` to `https://api.anthropic.com` and `accounts.google.com` (Google sign-in uses FedCM). Keep it that tight. Do not add inline `<script>`, inline `style="..."` attributes in HTML strings, or `eval`. Set dynamic styles via the CSSOM (`el.style.x = ...`) instead.
