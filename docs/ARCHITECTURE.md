...
| `auth.js` | Local accounts: register, login, logout, session restore, profile and settings updates, password change (PBKDF2-SHA256, 210k iterations, random salt), avatar processing (center crop, 256px re-encode to WebP/PNG), account deletion. |
...
| `salt`, `passHash` | string | Hex; PBKDF2-SHA256 |
